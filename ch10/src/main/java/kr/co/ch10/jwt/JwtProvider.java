...
public class JwtProvider {
    ...
    private SecretKey secretKey; // signature(서명)을 만들 때 secretKey를 사용
    ...
    public String createToken(User user, int days) {
        ...
        String token = Jwts.builder()
                ...
                .issuedAt(issuedDate) // 발급 시간
                .expiration(expireDate) // 만료 시간
                .claim("username", user.getUid()) // 사용자 정보 username
                .claim("role", user.getRole())   // 사용자 정보 role, 최소한의 정보만 저장해야 함
                .signWith(this.secretKey, Jwts.SIG.HS256) // secretKey로 서명
                .compact();

        return token;
    }

    // 토큰으로부터 클레임(username, role) 추출
